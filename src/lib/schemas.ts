...
export const setupSchema = z.object({ pin: pinSchema, profileName: z.string().trim().min(1).max(60), roomName: z.string().trim().min(1).max(80) });
export const joinSchema = z.object({ pin: pinSchema });
export const recoverSchema = z.object({ recoveryCode: z.string().trim().min(12).max(80), pin: pinSchema });
