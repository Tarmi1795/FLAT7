...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-dvh bg-background text-foreground">
      ...
      {isDashboard ? (
        <main id="main-content" className="mx-auto w-full max-w-[1680px] px-2 pb-28 pt-2 sm:px-4 sm:pt-4 lg:px-6 lg:pb-6 lg:pt-6">{children}</main>
      ) : (
        <div className="relative min-h-dvh overflow-hidden">
          <div className="app-ambient-bg" aria-hidden="true" />
          <div className="relative z-20 mx-auto w-full max-w-[1540px] px-3 pt-3 sm:px-5 sm:pt-5 lg:px-6 lg:pt-6">
            <div className="rounded-[26px] border border-white/[0.09] bg-[#07110c]/72 p-3 shadow-[0_28px_90px_rgba(0,0,0,.38)] backdrop-blur-xl sm:rounded-[32px] sm:p-5 lg:p-6"><TopBar /></div>
          </div>
          <main key={pathname} id="main-content" className="route-enter relative z-10 mx-auto w-full max-w-[1440px] px-4 pb-28 pt-7 sm:px-6 sm:pt-9 lg:px-8 lg:pb-12 lg:pt-10">{children}</main>
        </div>
      )}
      <QuickEntrySheet />
