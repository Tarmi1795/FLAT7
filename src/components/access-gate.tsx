...
export function AccessGate({ children }: { children: React.ReactNode }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (payload.recoveryCode) { setRecoveryCode(payload.recoveryCode); setLoading(false); return; }
  ...
  return (
    <main className="grid min-h-dvh place-items-center bg-[radial-gradient(circle_at_top,#153621_0%,#07110c_48%)] p-4">
      <section className="w-full max-w-md rounded-[28px] border border-white/10 bg-[#0e1a13]/95 p-6 shadow-2xl sm:p-8">
        <Brand />
        {recoveryCode ? (
          <div className="mt-8"><span className="grid size-12 place-items-center rounded-2xl bg-emerald-300/10 text-emerald-300"><ShieldCheck /></span><h1 className="mt-5 text-2xl font-bold text-white">Save your recovery code</h1><p className="mt-2 text-sm leading-6 text-slate-400">This is the only way to reset a forgotten household PIN. Store it somewhere private.</p><code className="mt-5 block rounded-2xl border border-emerald-300/20 bg-[#07110c] p-4 text-center text-lg font-bold tracking-[.16em] text-emerald-300">{recoveryCode}</code><button className="primary-button mt-5 w-full" onClick={() => { setAuthorized(true); window.dispatchEvent(new Event("flat7-access-changed")); }}>I saved it</button></div>
        ) : (
          <>
            <div className="mt-8"><p className="eyebrow">Private household</p><h1 className="mt-2 text-3xl font-bold tracking-tight text-white">Welcome home.</h1><p className="mt-2 text-sm leading-6 text-slate-400">Use the shared six-digit PIN to join FLAT7 on this device.</p></div>
            ...
            <form className="mt-5 space-y-4" onSubmit={submit}>
              {mode === "setup" && <><label className="block"><span className="field-label">Your name</span><input className="field-input mt-2" name="profileName" required maxLength={60} autoComplete="name" placeholder="e.g. Tariq" /></label><label className="block"><span className="field-label">First room</span><input className="field-input mt-2" name="roomName" required maxLength={80} placeholder="e.g. Living room" /></label></>}
