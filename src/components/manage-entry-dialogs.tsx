...
import type { ACUnit, ActivityItem, BillOccurrence, ExpenseKind, Plant, Profile, Room } from "@/types/homecare";
...
export function EditActivityDialog({ activity }: { activity: ActivityItem }) {
  ...
  const [open, setOpen] = useState(false); const [loading, setLoading] = useState(false); const [error, setError] = useState("");
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setLoading(true); setError(""); const form = new FormData(event.currentTarget); try { const occurredAt = new Date(String(form.get("occurredAt"))); if (occurredAt > new Date()) throw new Error("Future dates are not allowed."); await updateActivity(activity.id, { occurredAt: occurredAt.toISOString(), note: String(form.get("note") || "") || undefined, profileId: String(form.get("profileId")) }); setOpen(false); } catch (submitError) { setError(submitError instanceof Error ? submitError.message : "Unable to update this activity."); } finally { setLoading(false); } };
  const localDate = new Date(new Date(activity.occurredAt).getTime() - new Date().getTimezoneOffset() * 60_000).toISOString().slice(0, 16);
  const maxLocal = new Date(new Date().getTime() - new Date().getTimezoneOffset() * 60_000).toISOString().slice(0, 16);
  return <><button type="button" className="icon-button" onClick={() => setOpen(true)} aria-label={`Edit ${activity.title}`}><Pencil className="size-4" /></button><Modal open={open} title="Edit activity" eyebrow={activity.title} onClose={() => setOpen(false)}><form className="mt-6 space-y-4" onSubmit={submit}><label className="block"><span className="field-label">Date and time</span><input name="occurredAt" className="field-input mt-2" type="datetime-local" defaultValue={localDate} max={maxLocal} required /></label><label className="block"><span className="field-label">Person</span><select name="profileId" className="field-input mt-2" defaultValue={activity.profileId}>{data.profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.name}</option>)}</select></label><label className="block"><span className="field-label">Note</span><textarea name="note" className="field-input mt-2 min-h-24 py-3" defaultValue={activity.detail === "Completed" ? "" : activity.detail} maxLength={500} /></label>{error && <p className="text-sm text-rose-300" role="alert">{error}</p>}<SubmitButton loading={loading} label="Save changes" /></form></Modal></>;
}

export function BillDialog({ bill }: { bill?: BillOccurrence }) {
  const { data, addBill, updateBill } = useHomecare();
  const [open, setOpen] = useState(false); const [loading, setLoading] = useState(false); const [error, setError] = useState("");
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setLoading(true); setError(""); const form = new FormData(event.currentTarget); const input = { name: (bill?.name || String(form.get("name"))) as ExpenseKind, amount: Number(form.get("amount")), dueAt: String(form.get("dueAt")), responsibleProfileId: String(form.get("profileId") || "") || undefined }; try { if (bill) await updateBill(bill.id, input, String(form.get("scope")) as "month" | "future"); else await addBill(input); setOpen(false); } catch (submitError) { setError(submitError instanceof Error ? submitError.message : "Unable to save this bill."); } finally { setLoading(false); } };
  return <><button type="button" className={bill ? "icon-button" : "secondary-button"} onClick={() => { setError(""); setOpen(true); }} aria-label={bill ? `Edit ${bill.name} bill` : undefined}>{bill ? <Pencil className="size-4" /> : <><Plus className="size-4" />Add recurring bill</>}</button><Modal open={open} title={bill ? `Edit ${bill.name}` : "Add a recurring bill"} eyebrow="Monthly expense" onClose={() => setOpen(false)}><form className="mt-6 space-y-4" onSubmit={submit}><label className="block"><span className="field-label">Expense type</span>{bill ? <div className="field-input mt-2 flex items-center text-slate-300" aria-readonly="true">{bill.name}</div> : <select name="name" className="field-input mt-2"><option>Internet</option><option>Rent</option><option>Water &amp; Electricity</option></select>}</label><label className="block"><span className="field-label">Amount (QAR)</span><input name="amount" className="field-input mt-2" type="number" min="0" step="0.01" inputMode="decimal" defaultValue={bill?.amount} required /></label><label className="block"><span className="field-label">Due date</span><input name="dueAt" className="field-input mt-2" type="date" defaultValue={bill?.dueAt.slice(0, 10)} required /></label><label className="block"><span className="field-label">Responsible person</span><select name="profileId" className="field-input mt-2" defaultValue={bill?.responsibleProfileId || ""}><option value="">Everyone</option>{data.profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.name}</option>)}</select></label>{bill && <fieldset><legend className="field-label">Apply changes to</legend><div className="mt-2 grid grid-cols-2 gap-2"><label className="chip min-h-11 cursor-pointer justify-center"><input type="radio" name="scope" value="month" defaultChecked /> This month only</label><label className="chip min-h-11 cursor-pointer justify-center"><input type="radio" name="scope" value="future" /> This and future</label></div></fieldset>}<p className="rounded-xl border border-emerald-300/10 bg-emerald-300/[0.05] p-3 text-xs leading-5 text-emerald-100">Saved once, this expense repeats monthly for the next 12 months. Paid history is never changed.</p>{error && <p className="text-sm text-rose-300" role="alert">{error}</p>}<SubmitButton loading={loading} label={bill ? "Save changes" : "Create monthly series"} /></form></Modal></>;
}
