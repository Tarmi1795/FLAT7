  ...
  return (
    <div>
      ...
      <div className="mt-7 grid gap-6 xl:grid-cols-[1fr_.8fr]">
        <section className="panel p-5 sm:p-6"><div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-2xl bg-emerald-300/10 text-emerald-300"><UsersRound className="size-5" /></span><div><h2 className="text-lg font-bold text-white">People & rooms</h2><p className="mt-0.5 text-sm text-slate-400">Every room has a primary person.</p></div></div><div className="mt-5 space-y-3">{data.rooms.map((room) => { const person = data.profiles.find((item) => item.id === room.primaryProfileId); return <div key={room.id} className="flex items-center gap-3 rounded-2xl border border-white/[0.06] bg-white/[0.025] p-3"><span className="grid size-10 place-items-center rounded-xl bg-white/[0.04] text-slate-300"><MapPinHouse className="size-5" /></span><div className="min-w-0 flex-1"><p className="font-semibold text-white">{room.name}</p><p className="mt-0.5 text-xs text-slate-400">Primary person · {person?.name}</p></div><span className="grid size-8 place-items-center rounded-full text-xs font-bold text-[#07110c]" style={{ backgroundColor: person?.color }}>{person?.initials}</span></div>; })}</div></section>
